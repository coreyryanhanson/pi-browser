...
4 builtin pattern guides (`bot-detection`, `cookie-consent`, `pagination`, `search`). Site guides are user-authored — place a `.md` file with YAML frontmatter in `~/.pi/agent/pi-lean-portal/web-guides/` — and auto-register via their `domains` field. Caches invalidate on `web-learn` calls. Guides surface via an applicable-guide footer and badge; all matching guides are shown together — within a category, `kind:"api"` guides order before `kind:"web"` (host-first: API access is cheaper/lower-context than browsing), but both always surface with no suppression.

**Peer-package guide providers (portal-owned receiving contract).** `index.ts` exposes `globalThis.__piLeanPortalRegisterGuideProvider` = `registerGuideProvider` (from `core/guides.ts`). A co-installed peer (currently `pi-lean-host`) calls it once at load to register a provider that returns a fresh `Record<string, Guide>` snapshot on every `getGuideContent()` call. Provider output merges **below** user guides and **above** builtins; a throwing provider is skipped so one bad peer can't block the store. Peer guides carry `kind: "web" | "api"` (omitted = `web`) — the field is a presentation/ordering hint only; **no recipe-structured data crosses the boundary** (`apiHost`/`operations`/`pagination`/`auth` are stripped on the peer's side before it ever reaches portal). api-kind peer guides are namespaced under the key `api:<shortName>` so a same-named user web guide can't collide with them (a domain may have both a web guide and one or more api guides, all discoverable). In `formatGuideFooter`, api-kind guides route to the peer's tool — `api-guide({domain, guide})` — instead of `web-guide`; the `domain` field on `ApplicableGuide` is the guide's declared domain (so routing resolves by exact domain, not the navigated `www.` hostname). Portal never imports the peer; the projection is a runtime feature-detect from the peer's side. See `packages/pi-lean-host/AGENTS.md` for the sending-side projection.

**Domain matching.** `resolveApplicableGuides` matches a guide's declared domain against the exact hostname **or any subdomain of it** (`example.com` covers `www.example.com`), so a guide that lists only the apex still surfaces on subdomain navigations. A domain may match multiple guides (web + api); `sortApplicableGuides` dedups by name and applies the host-first ordering above.
